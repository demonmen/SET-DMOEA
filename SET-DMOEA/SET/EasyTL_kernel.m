function [acc,y_pred] = EasyTL_kernel(Xs,Ys,Xt,Yt,intra_align,dist,lp)
...
% IEEE International Conference on Multimedia & Expo (ICME) 2019.
    ...
    switch(intra_align)
        ...
        case 'gfk'
            ...
            [Xs,Xt,~] = GFK_map(Xs,Ys,Xt,Yt);
        case 'coral'
            %fprintf('EasyTL using CORAL...\n');
            Xs = CORAL_map(Xs,Xt);
    end
    Dct= get_Dct(Xs,Ys,Xt);
    %Dct 目标域任意样本与任意源域类心的距离
...
% Get source class center and Dct
    ...
    for i = class_set
        ...
        indx = Ys == i;
        X_i = Xs(indx,:);
        xxx1 = diag(kernel(ker, Xt', []));
        xxx2 = kernel(ker, X_i',Xt');
        xxx3 = mean(mean(kernel(ker, X_i', [])));
        Dct_c = sqrt(xxx1-2*mean(xxx2,1)'+xxx3);
        Dct = [Dct,Dct_c];
    end
end
